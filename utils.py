...
class TrendAnalyzer:
    def __init__(self):
        with open(r'C:\Users\nancy\Downloads\archive\company_tickers_exchange.json', 'r') as f:
            ...
            print(company_names)
        # the model is from: https://code.google.com/archive/p/word2vec/
        self.model = KeyedVectors.load_word2vec_format(r"GoogleNews-vectors-negative300.bin", binary=True)
    ...
    # Find n similar terms on a given semantic similarity metric
    def find_similar(self, n, metric, stock):
        similar_words = self.model.most_similar(stock, topn=n)
    ...
    # Builds trend data for the list of keywords found by semantic similarity
    def build_trend_data(self, keywords):
        X_values = []
        i = 0
        while i < len(keywords):
            kw = keywords[i]
            self.pytrends.build_payload([kw], timeframe='today 12-m')
            try:
    ...
    def prepare_data(self, df, close_values):
        X_values = []
        for col in df.columns:
            X_values.append(df[col].values)
    ...
    def train(self, X_train, Y_train, learning_rate=1e-3, loss='mse', metrics=['mae']):
        # train
        model = Sequential([
